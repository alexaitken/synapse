module Synapse::EventStore::Mongo
  ...
  class DocumentPerEventStrategy < StorageStrategy
    ...
    class EventDocument < SerializedDomainEventData
      # @return [String]
      attr_reader :id
      ...
      def to_events(aggregate_id, serializer, upcaster_chain)
        ...
        context = SerializedDomainEventUpcastingContext.new self, aggregate_id, serializer
        upcast_objects = upcaster_chain.upcast payload, context
        upcast_objects.each do |upcast_object|
          upcast_data = UpcastSerializedDomainEventData.new self, aggregate_id, upcast_object
