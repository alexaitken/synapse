module Synapse
  module EventSourcing
    ...
    class EventSourcingRepository < Persistence::LockingRepository
      ...
      def perform_load(aggregate_id, expected_version)
        begin
          ...
          raise Persistence::AggregateNotFoundError
        end
        ...
        if expected_version
          unless @conflict_resolver
            ensure_version_expected aggregate, expected_version
          end
        end
      ...
      def post_registration(aggregate)
        if @snapshot_policy && @snapshot_taker
          listener = SnapshotUnitListener.new type_identifier, aggregate, @snapshot_policy, @snapshot_taker
          register_unit_listener listener
        end
      end
